# app.py — SiReset (Streamlit) con “escudo anti-crash” y worker en background
from __future__ import annotations

import os, sys, json, time, uuid, shutil, traceback
...
# Mostrar trazas completas siempre
st.set_page_config(page_title="SiReset", layout="wide")
st.set_option("client.showErrorDetails", True)

# ============= ESCUDO ANTI-CRASH: wrapper global =============
def _fatal(e: BaseException):
    st.error("🚨 **Fallo crítico al ejecutar la app.**")
    st.code("".join(traceback.format_exception(e)), language="text")
    st.stop()

try:
    # ---------- Encabezado (si existe imagen) ----------
    try:
    ...
    for p in (APP_ROOT, CORE_DIR):
        sp = str(p)
        if sp not in sys.path:
            sys.path.insert(0, sp)

    # Asegurar que core sea paquete
    try:
        if CORE_DIR.exists():
            init_py = CORE_DIR / "__init__.py"
            if not init_py.exists():
                init_py.write_text("# auto-created\n", encoding="utf-8")
    except Exception:
        pass
    ...
    try:
        ...
        st.error(f"No pude importar auth.py: {e}")
        st.stop()

    # Resolver funciones del core
    def require_any(preferido: str):
        fn = getattr(mc, preferido, None)
        if callable(fn):
            return fn
        exports = sorted([x for x in dir(mc) if not x.startswith("_")])
        st.error(f"Falta función *{preferido}* en mougli_core. Exporta alguna de: {', '.join(exports)}")
        st.stop()
    ...
    def _unique_list_str(series, max_items=50):
        if series is None:
            return "—"
        vals = (
            series.astype(str).str.strip().replace({"nan": ""}).dropna().loc[lambda s: s.str.len() > 0].unique().tolist()
        )
        if not vals: return "—"
    ...
    def _web_resumen_enriquecido(df: Optional[pd.DataFrame], *, es_monitor: bool) -> pd.DataFrame:
        ...
        if df is not None and not df.empty:
            if cat_col: extras_rows.append({"Descripción": "Categorías (únicas)", "Valor": _unique_list_str(df[cat_col])})
            if reg_col: extras_rows.append({"Descripción": "Regiones (únicas)", "Valor": _unique_list_str(df[reg_col])})
            if tipo_col: extras_rows.append({"Descripción": "Tipos de elemento (únicos)", "Valor": _unique_list_str(df[tipo_col])})
    ...
    def _start_worker(job_dir: Path, mon_paths: List[Path], out_paths: List[Path],
        ...
        # -m core.mougli_core si es paquete, sino por ruta
        base_cmd = [sys.executable]
        if (CORE_DIR / "__init__.py").exists() and (CORE_DIR / "mougli_core.py").exists():
            base_cmd += ["-m", "core.mougli_core"]
        else:
        ...
        for p in mon_paths: args += ["--monitor", str(p)]
        for p in out_paths: args += ["--outview", str(p)]
    ...
    def _read_progress(job_dir: Path) -> Dict:
        ...
        if pj.exists():
            try: return json.loads(pj.read_text(encoding="utf-8"))
            except Exception: return {"status": "unknown", "message": "progress.json corrupto"}
        return {"status": "unknown", "message": "sin progreso"}

    def _read_log_tail(job_dir: Path, lines: int = 200) -> str:
        logf = job_dir / "job.log"
        if not logf.exists(): return ""
        try:
    ...
    user = current_user()
    if not user:
        # Muestra diagnóstico mínimo antes del login (para depurar arranque)
        with st.expander("Diagnóstico rápido del entorno"):
            st.write({"python": sys.version, "cwd": str(APP_ROOT), "core_dir": str(CORE_DIR)})
            st.write("Contenido de /core:", [p.name for p in CORE_DIR.glob("*")] if CORE_DIR.exists() else "(no existe)")
        login_ui()
        st.stop()
    ...
    if app == "Mougli":
        ...
        if st.sidebar.button("💾 Guardar factores"):
            save_monitor_factors(factores); save_outview_factor(out_factor)
    ...
    if app == "Mougli":
        ...
        if job_id:
            ...
            with colP:
                step = int(prog.get("step", 0)); total = int(prog.get("total", 6) or 6)
                st.progress(min(step, total) / max(total, 1), text=prog.get("message", "Procesando…"))
            with colBtns:
                if st.button("↻ Actualizar"): st.experimental_rerun()
                if st.button("Cancelar / Limpiar"):
                    _clear_job(job_id); st.experimental_rerun()
        ...
        if st.button("Procesar (seguro, en background)", type="primary"):
            ...
            try:
                mon_paths = _save_upload_to(job_dir, up_monitor_multi, "monitor")
                out_paths = _save_upload_to(job_dir, up_out_multi, "outview")

                if not mon_paths and not out_paths:
                    st.warning("Sube al menos un archivo de Monitor u OutView.")
                else:
                    _start_worker(job_dir, mon_paths, out_paths, factores, out_factor)
        ...
        st.markdown("---")
        st.markdown("### Vista rápida (sin procesar todo)")
        st.caption("Carga y muestra **resúmenes** para validar archivos (no genera Excel).")
        colA, colB = st.columns(2)
        with colA:
            if up_monitor_multi:
                try:
                    buf = BytesIO()
                    for i, f in enumerate(up_monitor_multi):
                        data = f.getvalue()
                        if i > 0: buf.write(b"\n")
                        buf.write(data)
        ...
        with colA:
            ...
            if ok_new:
                if not (c_email and c_name and c_pwd):
                    ...
                    try:
                        create_user(email=c_email, name=c_name, role=c_role, pwd=c_pwd,
