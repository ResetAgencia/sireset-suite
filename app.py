# app.py — SiReset (Streamlit) — robusto y compatible
import sys
...
st.set_option("client.showErrorDetails", True)  # mostrar traza completa
st.image("assets/Encabezado.png", use_container_width=True)

# Añadir rutas para importar core/*
...
try:
    ...
    st.error(f"No pude importar core.mougli_core:\n\n{e}")
    st.stop()

# Resolver funciones (existirán con estos nombres)
procesar_monitor_outview   = getattr(mc, "procesar_monitor_outview")
resumen_mougli            = getattr(mc, "resumen_mougli")
_read_monitor_txt         = getattr(mc, "_read_monitor_txt")
_read_out_robusto         = getattr(mc, "_read_out_robusto")
load_monitor_factors      = getattr(mc, "load_monitor_factors")
save_monitor_factors      = getattr(mc, "save_monitor_factors")
load_outview_factor       = getattr(mc, "load_outview_factor")
save_outview_factor       = getattr(mc, "save_outview_factor")
...
def llamar_procesar_monitor_outview(monitor_file, out_file, factores, outview_factor):
    try:
        ...
        # versiones anteriores sin outview_factor
...
HIDE_OUT_PREVIEW = {
    ...
    "Semana en Mes por Código",  # ¡ocúltala también en la vista previa!
...
def _preview_df(df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    drop_cols = [c for c in df.columns if c and c.strip().lower() in normalized_hide]
...
if app == "Mougli":
    ...
    if st.button("Procesar Mougli", type="primary"):
        try:
            # Combinar subidas
            def combinar_monitor_txt(files):
                ...
                for i, f in enumerate(files):
                    ...
                    if i > 0: buf.write(b"\n")
                    buf.write(data)
                buf.seek(0); setattr(buf, "name", "monitor_combined.txt")
                try:
                    df_m = _read_monitor_txt(buf); buf.seek(0)
                except Exception:
                    df_m = None
                return buf, df_m

            def _read_out_file_to_df(upload):
                ...
                try:
                    if name.endswith(".csv"):
                        try:
                            ...
                            upload.seek(0); return pd.read_csv(upload, sep=";", encoding="latin-1", low_memory=False)
            ...
            def combinar_outview(files):
                ...
                out = BytesIO(); dfc.to_csv(out, index=False); out.seek(0); setattr(out, "name", "outview_combined.csv")
                return out, dfc
            ...
            with colA:
                ...
                if df_m_res is None and mon_proc is not None:
                    try:
                        mon_proc.seek(0); df_m_res = _read_monitor_txt(mon_proc); mon_proc.seek(0)
                    except Exception:
                        df_m_res = None
            ...
            with colB:
                ...
                if df_o_res is None and out_proc is not None:
                    try:
                        out_proc.seek(0); df_o_res = _read_out_robusto(out_proc); out_proc.seek(0)
                    except Exception:
                        df_o_res = None
